...
public class ProcessContractProviderTests
{
    [Fact]
    public async Task EncryptedFileProvider_SendReceive_RoundTrips()
    {
        ...
        try
        {
            var provider = new EncryptedFileProcessContractProvider(tmpDir);
            var info = new ProcessContract(
                ...
                downloadTimeOut: 30,
                appSecretKey: "secret",
                updateVersions: new List<VersionEntry> { new() { Version = "1.0.0", Name = "test" } },
            ...
            Assert.NotEmpty(result.Formats);
