...
public class DefaultBlackListMatcherTests
{
    private static BlackListConfig CreateConfig(List<string> files = null, List<string> formats = null, List<string> dirs = null)
        => new(files, formats, dirs);

    [Fact]
    public void IsBlacklisted_ExactFileNameMatch_ReturnsTrue()
    {
        var matcher = new DefaultBlackListMatcher(CreateConfig(
    ...
    [Fact]
    public void FromConfigInfo_BlackFilesEmpty_UsesNullInConfig()
    {
        var config = new GlobalConfigInfo { BlackFiles = new List<string>() };
        var matcher = DefaultBlackListMatcher.FromConfigInfo(config);
