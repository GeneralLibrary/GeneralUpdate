...
public sealed class VersionHub<TParameter> where TParameter : class
{
    ...
    /// <param name="receiveMessageCallback">
    ///     Receive server push callback function, The caller needs to implement the update
    ///     process.
    /// </param>
    ...
    public void Subscribe(string url, string name, Action<TParameter> receiveMessageCallback,
        Action<string> onlineMessageCallback = null, Action<string> reconnectedCallback = null)
    {
        if (string.IsNullOrWhiteSpace(url) || receiveMessageCallback == null)
            throw new Exception("Subscription key parameter cannot be null !");
    ...
    private void OnReceiveMessage(string name, string message)
    {
        ...
        try
        {
            var clientParameter = JsonSerializer.Deserialize<TParameter>(message);
            if (clientParameter == null)
                throw new ArgumentNullException(
                    $"'VersionHub' Receiving server push version information deserialization failed , receive content :  {message} .");
    ...
    private async Task OnClosed(Exception arg)
    {
        ...
        {
            if (arg != null)
                throw new Exception($"'VersionHub' On closed internal exception :  {arg.Message}", arg.InnerException);
